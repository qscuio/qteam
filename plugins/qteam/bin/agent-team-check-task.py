...
def main():
    ...
    tdd_missing_seams = []
    if args.task and rec.get("tdd_required"):
        expected_seams = {seam.get("id") for seam in rec.get("test_seams", [])
                          if isinstance(seam, dict)}
        evidenced_seams = {item.get("seam_id") for item in rec.get("tdd_evidence", [])
                           if isinstance(item, dict)}
        tdd_missing_seams = sorted(expected_seams - evidenced_seams)
    tdd_missing = bool(tdd_missing_seams)
    diagnosis_missing = bool(args.task and rec.get("diagnosis_required")
                             and not rec.get("diagnosis_evidence"))
    debug_markers = []
    if args.task and entries:
        paths = [path for _status, path in entries]
        marker = subprocess.run(
            ["git", "grep", "-n", "-E", r"\[QTEAM-DEBUG-[^]]+\]",
             head_sha, "--", *paths], cwd=repo, text=True,
            stdout=subprocess.PIPE, stderr=subprocess.PIPE,
        )
        if marker.returncode not in {0, 1}:
            sys.stderr.write(marker.stderr)
            sys.exit(2)
        debug_markers = marker.stdout.splitlines()
    ...
    ok = not (violations or forbidden_hits or shared_hits or dirty or
              empty_violation or verification_missing or tdd_missing
              or diagnosis_missing or debug_markers)
    ...
    if tdd_missing:
        print("TDD evidence missing for approved seam(s): "
              + ", ".join(tdd_missing_seams))
    if diagnosis_missing:
        print("DIAGNOSIS evidence missing (need structured feedback-loop/root-cause report)")
    if debug_markers:
        print("DEBUG instrumentation remains in task HEAD:")
        for line in debug_markers:
            print(f"  {line}")
    if dirty:
        ...
        res = subprocess.run(
            [sys.executable, str(state_tool), "--run", args.run, "task-check",
             ...
             "--evidence", (f"{len(entries)} changed paths; "
                            ...
                            f"tdd_missing={tdd_missing} diagnosis_missing={diagnosis_missing} "
                            f"debug_markers={len(debug_markers)}")],

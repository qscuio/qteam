...
Run from the qnote repository root:
  ...
  <target-repo>/.codex/bin/import-agent-learning <target-repo> <run-id> [--dry-run] [--update]
    ...
    for item in manifest["items"]:
        ...
        for field in ("validation_scope", "claim_boundary"):
            ...
            if (not isinstance(value, str) or not value.strip()
                ...
                invalid_boundary = field
                break
            boundaries[field] = value.strip()
        if invalid_boundary:
        ...
        boundary = (
            f"> Validation scope: {boundaries['validation_scope']}\n"
            f"> Claim boundary: {boundaries['claim_boundary']}\n\n"
        )
        if args.dry_run:
            imported.append((title, dst, True))
        else:
            dst.parent.mkdir(parents=True, exist_ok=True)
            dst.write_text(header + boundary + content, encoding="utf-8")

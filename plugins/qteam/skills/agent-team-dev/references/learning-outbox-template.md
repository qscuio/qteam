  ...
  "items": [
    {
      ...
      "validation_scope": "middleware-order behavior covered by tests/auth/order.test.ts at commit def456",
      "claim_boundary": "does not establish ordering for middleware outside the tested auth chain",
...
- Every approved item has non-empty single-line `validation_scope` and
  `claim_boundary` fields. The importer rejects missing or multiline boundaries
  and writes both into the durable note.

...
Title:
Agent:
Work kind:        (feature / bugfix / debug / refactor / test / integration / docs / config / generated / learning / experiment)
Risk flags:       (factual; normally empty)
Parallel group:
Depends on:
...
Contract:
Approved test seams: (feature/bugfix only; each: id, behavior, test paths, focused command, RED pattern)
Scenario coverage:   (feature/bugfix only; all 12 dimensions, one strongest applicable case or rationale)
Required decisions:  (exact decision-gate IDs; normally empty)
Typed handoff:       (successor / user-decision / replan / no-followup, when material)
...
- Dependency IDs and wave placement are frozen after registration. Change them
  only by entering `REPLANNING` and replacing the task record.
- A task may start only after every `depends_on` task is `merged` or
  `artifact_complete`. Wave start checks all selected tasks, and task start
  repeats the gate under the state lock.
...
- [ ] P0-A Define shared interface
  ...
  - Work kind: feature
  - Risk flags: public-api, compatibility
  ...
  - Approved test seams:
    - id: exported-contract
      behavior: downstream modules typecheck against the stable public contract
      test paths: tests/contracts/exported-contract.test.ts
      command: npm test -- contracts
      RED pattern: contract fixture does not compile
  - Tests:
    - typecheck
  - Verification:
    - npm run typecheck
...
- [ ] P1-A Implement auth middleware
  - Agent: developer
  - Work kind: feature
  - Risk flags: authentication, authorization
  ...
  - Approved test seams:
    - id: request-auth
      behavior: unauthenticated requests are rejected at the middleware boundary
      test paths: tests/auth/request-auth.test.ts
      command: npm test -- auth
      RED pattern: expected 401
  - Tests:
...
- [ ] P1-B Implement billing adapter
  ...
  - Approved test seams:
    - id: billing-adapter
      behavior: callers receive the normalized charge result
      test paths: tests/billing/adapter.test.ts
      command: npm test -- billing
      RED pattern: expected normalized charge
  - Tests:

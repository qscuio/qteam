...
assets, no executable attributes,
no scripts other than the one canonical motion controller), and — when motion
markup is present — the structural motion contract. Pair it with the packaged
`verify_geometry.py` check for SVG label-mask clipping.
"""
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        for key, value in normalized_attrs:
            ...
            if key in REFERENCE_ATTRS:
                self.references.append((tag, key, data.get("rel", ""), value))
...
def reference_error(tag: str, attribute: str, rel: str, value: str) -> str | None:
    stripped = value.strip()
    lowered = stripped.casefold()
    if not stripped or stripped.startswith("#"):
        return None
    if attribute == "srcset":
        return f"srcset is not allowed in a single-file diagram on <{tag}>"
    ...
    return f"sidecar reference on <{tag}> is not self-contained: {stripped[:80]}"
...
def verify(path: Path) -> list[str]:
    ...
    for tag, attribute, rel, value in parser.references:
        finding = reference_error(tag, attribute, rel, value)
        if finding:
            errors.append(finding)
    style_sources = parser.styles + parser.inline_styles
    style_source = "".join(style_sources)
    if re.search(r"@import\b", style_source, re.IGNORECASE):
        errors.append("CSS @import is not allowed in a self-contained diagram")
    if any(CSS_REMOTE_RE.search(css) for css in style_sources):
        errors.append("remote CSS references are not allowed in a self-contained diagram")
    for css in style_sources:
        for match in re.finditer(r"url\(\s*(['\"]?)(.*?)\1\s*\)", css,
                                 re.IGNORECASE | re.DOTALL):
            value = match.group(2).strip().casefold()
            if not value.startswith(("#", "data:image/")):
                errors.append(
                    "CSS sidecar references are not allowed in a self-contained diagram"
                )
        if re.search(r"(?:-webkit-)?image-set\s*\(", css, re.IGNORECASE):
            errors.append("CSS image-set is not allowed in a self-contained diagram")

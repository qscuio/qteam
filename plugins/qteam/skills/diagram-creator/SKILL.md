...
description: Create editorial technical, product, data, process, chart, architecture, and UML diagrams as self-contained HTML/SVG/PNG. Use for UML class, use-case, component, deployment, activity, sequence, and state diagrams; for draw.io or Mermaid redraws; and when a visual communicates structure better than prose. Do not use for generic file display or interactive teaching experiences.
---
...
**Before generating your first diagram in a new project, resolve its external style profile.**

Don't silently ship default-skinned diagrams into a branded project.

First check the project root for a `.diagram-design` marker and resolve it per [`references/profiles.md`](references/profiles.md). A valid marker whose profile exists selects that file directly and skips this gate; `profile: default` also skips it. A malformed or missing-profile marker follows the visible failure handling in that reference. The installed `references/style-guide.md` is immutable package data and is never rewritten.

Without a valid marker, **pause and ask the user**:
...
After onboarding, offer to save the approved tokens as a named external profile and select it with the project marker. For unattended work, the delegation packet must freeze `style-profile: <slug>` or `style-profile: default`; never strand an AFK worker at this gate.
...
Use for any of the 32 visual types (§3) when a reader will learn more from a visual than from prose, a table, or a bulleted list.

**Don't use for:**

- Quick unicode diagrams → write a compact text diagram directly.
...
For an explicit UML request, first load
[`references/uml-notation.md`](references/uml-notation.md), then load the
matching type reference. UML owns the relationship meanings and notation;
the type reference and this skill own layout, accessibility, and editorial
quality. The output is HTML + inline SVG, not PlantUML source or an XMI model.

| If you're showing… | Use | Reference |
|---|---|---|
| Software classifiers, members, interfaces, and typed relationships | **UML class** | [type-uml-class.md](references/type-uml-class.md) |
| Actors and system-visible goals | **UML use case** | [type-uml-use-case.md](references/type-uml-use-case.md) |
| Components, ports, provided/required interfaces, and dependencies | **UML component** | [type-uml-component.md](references/type-uml-component.md) |
| Runtime nodes, execution environments, artifacts, and communication paths | **UML deployment** | [type-uml-deployment.md](references/type-uml-deployment.md) |
| Actions, decisions, forks, object flow, and partitions | **UML activity** | [type-uml-activity.md](references/type-uml-activity.md) |
| Lifelines, messages, activations, and bounded combined fragments | **UML sequence** | [type-sequence.md](references/type-sequence.md) |
| States, guarded transitions, entry/do/exit behavior | **UML state machine** | [type-state.md](references/type-state.md) |

Do not claim full UML conformance. Use only the supported subset documented in
the selected reference, preserve semantic distinctions, and disclose any
requested construct that was simplified or omitted.
...
| Classifiers + attributes + operations + UML relationships | **UML class** | [type-uml-class.md](references/type-uml-class.md) |
| Actors + goals inside a system boundary | **UML use case** | [type-uml-use-case.md](references/type-uml-use-case.md) |
| Components + ports + provided/required interfaces | **UML component** | [type-uml-component.md](references/type-uml-component.md) |
| Runtime nodes + deployed artifacts + communication paths | **UML deployment** | [type-uml-deployment.md](references/type-uml-deployment.md) |
| Actions + decisions + forks + object flow | **UML activity** | [type-uml-activity.md](references/type-uml-activity.md) |
...
**The design system is skinnable.** The immutable packaged defaults live in [`references/style-guide.md`](references/style-guide.md), which describes semantic roles (`paper`, `ink`, `muted`, `accent`, `link`, …). To apply a brand, create an external profile through [`references/onboarding.md`](references/onboarding.md) and select it with the project marker. Never edit the installed style guide.

> When specs below or in type references mention "ink", "accent", "muted", etc., look up the value in the resolved external profile, or in the packaged style guide when `profile: default` is selected.
...
- **Title** — Instrument Serif when locally available, then Georgia/serif,
  1.75rem, 400 — H1 only
- **Node name** — Geist when locally available, then system sans, 12px, 600
- **Sublabel** — Geist Mono when locally available, then system mono, 9px
- **Eyebrow / tag** — the same local/system mono stack, 7–8px, uppercase
- **Arrow label** — the same local/system mono stack, 8px
- **Editorial aside** — the local/system serif stack in italic, 14px
...
Do not add a remote font stylesheet. Offline output uses the declared local
family when installed and the packaged system fallbacks otherwise; disclose a
fallback when a supplied brand font is not locally available.
...
| Max entities (ER) | 8 |
| Max classifiers (UML class) | 7; max 6 visible members each |
| Max use cases / actors | 9 / 4 |
| Max UML components / interfaces | 9 / 8 |
| Max UML deployment nodes / artifacts | 8 / 10 |
| Max UML activity actions / decisions | 9 / 3 |
...
- [ ] For UML, loaded `uml-notation.md`, preserved every relationship's notation, and disclosed any unsupported construct?
...
- [ ] Ran the packaged geometry check — `python3 <skill-dir>/scripts/verify_geometry.py <file>` — clean? (Detects label masks clipped by later-painted nodes.)
- [ ] If animated, does the complete static/no-JS frame work, does reduced motion hide/disable playback, and is the controller copied verbatim from `template-motion.html`? Manually check print and static-query states on top of the packaged self-check.

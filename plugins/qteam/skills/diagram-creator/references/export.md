...
3. Make it standalone:
   ...
   - Preserve the source's local/system font-family declarations. Do not inject
     `@import`, remote font URLs, or a second `<defs>` block.
...
- **Preferred local font is unavailable**: Playwright selects the documented
  system fallback. Disclose that environment-specific substitution; do not add
  a remote dependency during export.

...
**The immutable packaged defaults for colors, typography, and tokens.** A diagram using `profile: default` draws from this file; a branded diagram draws from the external profile selected by `.diagram-design`. Never edit this installed file.

The default skin is a cool editorial palette — white-smoke paper, jet-black ink, atomic-tangerine accent, blue-slate muted. It is designed to work out of the box. Use [`onboarding.md`](onboarding.md) to create a branded external profile without changing package files.
...
Use local/system stacks only: `"Instrument Serif", Georgia, serif` for title
and callout roles, `Geist, Inter, ui-sans-serif, system-ui, sans-serif` for
names, and `"Geist Mono", "SFMono-Regular", Consolas, monospace` for technical
labels. Do not add a remote font stylesheet. If the preferred local family is
unavailable, disclose the rendered fallback rather than claiming an exact
brand match.
...
Keep this file unchanged. To customize:

1. **Run onboarding** — see [`onboarding.md`](onboarding.md); it extracts and previews an external profile.
2. **Map manual tokens** — put approved values into a new external profile, never this file.
3. **Brand handoff** — map supplied design-token JSON to the semantic roles in an external profile.
4. **Select a profile** — bind it to a project using [`profiles.md`](profiles.md).

...
def build_packet(repo, run_dir, task, role, extra_prompt):
    ...
    handoff = ("Before exit, leave the outbox uncommitted for controlled harvest and print a bounded digest."
               ...
               "Before exit, commit task changes locally on the assigned task branch and print a bounded digest with commits, files, verification commands, and results.")
    return f"""{prompt}
...
def cmd_spawn(args, repo, run_dir):
    ...
    while time.monotonic() < deadline:
        ...
        if record.get("pid") == wrapper.pid and record.get("state") == "running":
            break
        time.sleep(0.02)
    else:
    ...
    with open(args.stdout, "w", encoding="utf-8") as out, \
        ...
        try:
            child = subprocess.Popen(
                ["codex", "exec", "-C", args.cwd, "--sandbox", "workspace-write", prompt],
...
def parser():
    ...
    for flag in ("record", "result", "stdout", "stderr", "cwd", "prompt", "token", "workers"):
